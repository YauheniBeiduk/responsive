...
import { CreatePersona, UpsertPersona } from './persona-services/persona-service';
...
class PersonaState {
  ...
  public newPersonaAndOpen(persona: { name: string; description: string; personaMessage: string }) {
    this.persona = {
      ...this.defaultModel,
      name: persona.name,
...
export const addOrUpdatePersona = async (previous: any, formData: FormData) => {
  ...
  const response =
    model.id && model.id !== '' ? await UpsertPersona(model) : await CreatePersona(model);
