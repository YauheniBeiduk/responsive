...
export const FindPersonaByID = async (id: string): Promise<ServerActionResponse<PersonaModel>> => {
  try {
    ...
    const { resources } = await HistoryContainer().items.query<PersonaModel>(querySpec).fetchAll();
...
export const CreatePersona = async (
  ...
  try {
    ...
    const { resource } = await HistoryContainer().items.create<PersonaModel>(modelToSave);
...
export const UpsertPersona = async (
  ...
  try {
    ...
    if (personaResponse.status === 'OK') {
      ...
      const { resource } = await HistoryContainer().items.upsert<PersonaModel>(modelToUpdate);
