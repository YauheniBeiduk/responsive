...
import { PROMPT_ATTRIBUTE, PromptModel, PromptModelSchema } from '@/features/prompt-page/models';
...
export const CreatePrompt = async (
  ...
  try {
    ...
    const { resource } = await ConfigContainer().items.create<PromptModel>(modelToSave);
...
export const FindAllPrompts = async (): Promise<ServerActionResponse<Array<PromptModel>>> => {
  try {
    ...
    const { resources } = await ConfigContainer().items.query<PromptModel>(querySpec).fetchAll();
...
export const FindPromptByID = async (id: string): Promise<ServerActionResponse<PromptModel>> => {
  try {
    const querySpec: SqlQuerySpec = {
  ...
  try {
    ...
    if (promptResponse.status === 'OK') {
      ...
      const modelToUpdate: PromptModel = {
        ...
        isPublished: user.isAdmin ? promptInput.isPublished : prompt.isPublished,
        createdAt: new Date(),
      };
      ...
      const { resource } = await ConfigContainer().items.upsert<PromptModel>(modelToUpdate);
