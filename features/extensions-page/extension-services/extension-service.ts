...
import { EXTENSION_ATTRIBUTE, ExtensionModel, ExtensionModelSchema } from './models';
...
export const CreateExtension = async (
  ...
  try {
    ...
    if (validatedFields.status === 'OK') {
      ...
      const { resource } = await HistoryContainer().items.create<ExtensionModel>(modelToSave);
  ...
  try {
    ...
    if (extensionResponse.status === 'OK') {
      ...
      if (validatedFields.status === 'OK') {
        ...
        const { resource } = await HistoryContainer().items.upsert<ExtensionModel>(inputModel);
...
const validateFunctionSchema = (model: ExtensionModel): ServerActionResponse => {
