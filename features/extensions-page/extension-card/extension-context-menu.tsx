...
export const ExtensionCardContextMenu: FC<Props> = (props) => {
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger>
          {isLoading ? <LoadingIndicator isLoading={isLoading} /> : <MoreVertical size={18} />}
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          <DropdownMenuItemWithIcon onClick={() => extensionStore.openAndUpdate(props.extension)}>
            <Pencil size={18} />
            <span>Edit</span>
...
const useDropdownAction = (props: { extension: ExtensionModel }) => {
  ...
  const handleAction = async (action: DropdownAction) => {
    ...
    switch (action) {
      case 'delete':
        if (window.confirm(`Are you sure you want to delete ${extension.name}?`)) {
