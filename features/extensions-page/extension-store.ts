...
import { CreateExtension, UpdateExtension } from './extension-services/extension-service';
import {
  EndpointType,
...
class ExtensionState {
  ...
  public removeFunction(id: string) {
    this.extension.functions = this.extension.functions.filter((f) => f.id !== id);
  }
  ...
  public toggleFunction(functionId: string) {
    const functionToToggle = this.extension.functions.find((f) => f.id === functionId);
    if (functionToToggle) {
  ...
  public removeHeader(props: { headerId: string }) {
    ...
    this.extension.headers = this.extension.headers.filter((h) => h.id !== headerId);
  }
}
...
export const AddOrUpdateExtension = async (previous: any, formData: FormData) => {
